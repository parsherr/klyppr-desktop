...
`;
    ...
    data.result.forEach(segment => {
        const start = formatAssTime(segment.start_time);
        const end = formatAssTime(segment.end_time);
...
async function generateSubtitles(videoPath, apiKey, tempDir) {
    if (!apiKey) {
        throw new Error('API Key is required for generating subtitles.');
    }
    ...
    try {
        ...
        // 2. Upload to SpeechFlow and get transcription
        ...
        // 3. Convert to ASS and save
        const assContent = toAss(transcript);
